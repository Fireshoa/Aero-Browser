# Aero Browser 🚀
A lightweight, PC-style browsing experience built for mobile and Chromebooks.

## 🌟 Features
- **PC Emulation:** Renders sites with a Win11 desktop User-Agent.
- **Smart Filtering:** Built-in JS injection to monitor and block restricted keywords.
- **Command Bar:** Execute browser actions directly from the URL field.

## ⌨️ Commands
| Command | Action |
| :--- | :--- |
| `/print` | Opens the system print dialog for the current page. |
| `/disableblocker` | Suspends the `check()` interval for the current session. |
| `/enableblocker` | Resumes the content filtering script. |

## 🛠️ Built With
- **MIT App Inventor**
- **CustomWebView Extension** (by Sunny Gupta)
- **Custom JavaScript Injection**
- **Gemini** (For making the README.md look somewhat fucking decent)
